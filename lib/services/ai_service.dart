...
class AIService implements LLMInterface {
  ...
  Future<void> _loadModel() async {
    ...
    try {
      ...
      _model = _bindings.llama_load_model_from_file(modelPathUtf8.cast(), params);
  ...
  Future<String> generateText(String prompt, {
    ...
    try {
      ...
      final tokens = calloc<Int>(tokensCapacity);
      ...
      // Evaluate prompt
      _bindings.llama_eval(_context!, tokens, nTokens, 0, 1);
      ...
      for (var i = 0; i < maxTokens; i++) {
        // Sample token
        final token = _bindings.llama_sampling_sample(state, _context!, nullptr);
        ...
        // Get token text
        final tokenText = _bindings.llama_token_to_piece(_context!, token, nullptr, 0);
  ...
  Future<String> generateTextWithImage(String prompt, List<int> imageData, {
    ...
    try {
      ...
      final imageBuffer = imagePtr.asTypedList(imageData.length);
      imageBuffer.setAll(0, imageData);
      
      // Add image to context
      final imageAdded = _bindings.llama_add_image_data(
        _context!,
        imagePtr.cast(),
        imageData.length,
        0, // Image format (0 for auto-detect)
        0, // Width (0 for auto-detect)
        0, // Height (0 for auto-detect)
      );
      
      if (imageAdded != 0) {
        calloc.free(imagePtr);
        return 'Failed to add image to context';
      }
